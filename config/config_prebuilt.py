...
sys.path.append(os.path.join(os.path.dirname(os.path.dirname(__file__)), "bootimgpack", "pull"))
from andprop import andprop

PORT_ROOT = os.getenv("PORT_ROOT", os.path.dirname(os.path.dirname(__file__)))
DEFAULT_CERT_FILE = os.path.join(PORT_ROOT, "baidu", "release", "META", "apkcerts.txt")
...
class prebuilt(object):
    ...
    def __init__(self, out="prebuilt.mk", bsys=None, vsys=None):
        ...
        '''
        if bsys is None:
            bsys = os.path.join(PORT_ROOT, "baidu/release/system")
    ...
    def __out__(self):
        ...
        self.mOutStr = self.mOutStr + "BAIDU_PREBUILT_DIRS += \\\n"
