...
then
    ...
    echo "       ./sign.sh sign.zip *.zip      - to sign all apks for the unzip-ed zip-file"
    echo "       ./sign.sh sign.dir dir        - to sign all apks for the unzip-ed zip-file in dir"
...
then
    ...
    echo Siging Complete
    zippath=$(cd "$(dirname "$2")"; pwd)
    cd $ZIP_DIR
    zip -q -r -y "$2.sign.zip" *
    mv $2.sign.zip $zippath/
    cd -
    rm -rf $ZIP_DIR
    exit 0
fi
