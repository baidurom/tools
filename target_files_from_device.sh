...
function check_for_env_prepare {
	if [ ! -f $PRJ_ROOT/boot.img ];then
		echo ">>> can not find $PRJ_ROOT/boot.img"
		exit 1
	fi
...
function get_device_system_info {
	...
	if adb root | grep "cannot run" > /dev/null;then
		echo ">>> system is not kernel root, check weather it can run in su mode"
		...
		if echo "su < /data/local/tmp/exit_command; exit" | adb shell | grep "not found" > /dev/null;then
			echo ">>> ERROR: can not find su command, please root this device firstly"
			exit 1;
		else
			wait_for_device_online
